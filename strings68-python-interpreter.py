...
class Strings68Interpreter:
    ...
    def evaluate_expression(self, expr: str, loop_vars: Dict[str, str]) -> str:
        ...
        # shiftl(string, positions)
        if expr.startswith('shiftl('):
            args = self.extract_function_args(expr, 'shiftl', 2)
            string_val = self.evaluate_expression(args[0], loop_vars)
        ...
        if expr.startswith('shiftr('):
            args = self.extract_function_args(expr, 'shiftr', 2)
            string_val = self.evaluate_expression(args[0], loop_vars)
            positions = int(args[1].strip('"'))
            if len(string_val) == 0:
                return string_val
        ...
        if expr.startswith('rotl('):
            args = self.extract_function_args(expr, 'rotl', 2)
            string_val = self.evaluate_expression(args[0], loop_vars)
            positions = int(args[1].strip('"'))
            if len(string_val) == 0:
                return string_val
            positions = positions % len(string_val)  # Handle positions > length
            return string_val[positions:] + string_val[:positions]
        
        # rotr(string, positions) - rotate right
        if expr.startswith('rotr('):
            args = self.extract_function_args(expr, 'rotr', 2)
            string_val = self.evaluate_expression(args[0], loop_vars)
            positions = int(args[1].strip('"'))
            if len(string_val) == 0:
                return string_val
            positions = positions % len(string_val)  # Handle positions > length
            return string_val[-positions:] + string_val[:-positions] if positions > 0 else string_val
